...
- `src/utils/` — общие утилиты:
  ...
  - `groups.ts` — загрузка и инверсия групп пользователей;
...
`public/groups.json` загружается как ресурс расширения и превращается в lookup вида:
...
Этот lookup используется при трансформации ленты.
