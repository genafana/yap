...
При трансформации ленты для каждого пользователя строится список разрешённых тегов (`UserTagsLookup`) и отображается в виде цветных pill-меток в левой колонке коммента. Пользователи с тегом `ignore: true` скрываются полностью.
