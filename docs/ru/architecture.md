...
- `src/utils/` — общие утилиты:
  ...
  - `tags.ts` — хранение и работа с тегами пользователей (`TagsMap`, `UsersMap`);
...
- `TagsMap` — `Record<string, { bgColor?, ignore?, primary? }>` — определения тегов.
- `UsersMap` — `Record<string, { tags: string[] }>` — привязка пользователей к тегам.

Служебный тег `Игнор` поддерживается как защищённый системный тег (всегда существует, `ignore: true`, `primary: true`).

При трансформации ленты для каждого пользователя строится список разрешённых тегов (`UserTagsLookup`) и отображается в виде цветных pill-меток в левой колонке коммента. Пользователи с тегом `ignore: true` скрываются полностью. Первый `primary`-тег с цветом используется для подсветки пользовательского блока/левой колонки (в зависимости от настройки `primary_tag_full_user_bg`).

Импорт/экспорт тегов и пользователей доступен на странице настроек. Поддерживается импорт в нативном формате (version: 1) и в устаревшем формате.
...
2. `yap.content` на `document_end`:
   ...
   - загружает lookup тегов пользователей из extension storage;
