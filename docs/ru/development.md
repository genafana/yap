...
- системный тег **`Игнор`** всегда существует и защищён: его нельзя удалить, переименовать или «влить в» другой тег;
- у тега есть флаг `primary` (основной), который участвует в подсветке пользователя;
- настройка **«Общая подсветка пользователя»** (`primary_tag_full_user_bg`) переключает режим подсветки:
  - выключено: подсвечивается только блок пользователя;
  - включено: подсвечивается весь левый столбец комментария;
- **Импорт** — принимает нативный формат (`version: 1`) или устаревший формат (`groups.json`-стиль). Режим «добавить» или «заменить» выбирается галкой.
